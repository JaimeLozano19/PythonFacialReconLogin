...
while True:
    ...
    for(x,y,e1,e2) in cara:
        ...
        if resultado[1]<8000:
            cv.putText(captura,'Acceso autorizado', (x,y-40), 1,1.1(0,255,0),1,cv.LINE_AA)
            cv.putText(captura, '{}'.format(listaData[resultado[0]]), (x,y-20), 2,1.1,(0,255,0),1,cv.LINE_AA)
            cv.rectangle(captura, (x,y), (x+e1,y+e2), (255,0,0),2)
            cv.putText(captura,'Presione la tecla u', (x,y+180), 1,1.1(0,255,0),1,cv.LINE_AA)
            cv.putText(captura,'Para continuar', (x,y+200), 1,1.1(0,255,0),1,cv.LINE_AA)
            if cv.waitKey(1)==ord('u'):
                camara.release()
